...
\label{ch:qml}
How to combine quantum computing and machine learning is not easily answered. In the discussion of quantum machine learning, it is standard practice to reference the four quadrants of \cref{tab:qml_quadrants}, first described by Schuld and Petruccione \cite{textbook}.
...
Classical data being processed on classical computers is classical machine learning. Though not explicitly linked to quantum computing, there are some ways in which quantum computing influence classical machine learning. For example, the quantum inspired application of tensor networks in \cite{felser2020}.
...
More recently, Zeng et al. have explored a hybrid model to multi-class classification on real world datasets using a CNN-inspired structure \cite{zeng2022}. There it is shown that the hybrid model outperforms a classical CNN of similar parameter size.
% Use quantum layer in bigger model. Maybe beneficial? Use 'late' so diemnsions can be reduced and feasible for NISQ
