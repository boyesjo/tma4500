...
\footnotetext{
    ...
    It is effectively a column vector.  The inner product may be taken with a bra, $\bra{\cdot}$, to give a scalar.
...
First, it should be noted that states on the form \eqref{eq:qubit} are not unique, only the relative complex phase matters.
...
Interpreting $\theta$ as the polar angle and $\phi$ the azimuthal angle, the state of the qubit can be identified with a point on a sphere, see Figure~\ref{fig:bloch}.
The state $\ket{0}$ is typically thought of as the north pole, and $\ket{1}$ as the south pole of this sphere.
...
The $p_i$ must be non-negative and sum to one.
