...
How such large models avoid overfitting is not entireliy clear.
Seemingly concradicting the bias-variance tradeoff, the double descent phenomenon appears as model complexity increases.
...
\begin{table}
    ...
    \caption{
        ...
        However, in the case of the softmax function, it is applied to the entire output layer.
    }
    \label{tab:activations}
    \SetTblrInner{rowsep=3pt}
    \begin{tblr}{
            width=\linewidth,
            colspec={Q[l,m,co=1] Q[l,m,co=1,2] Q[r,m,co=1, 2]},
        }
        \toprule
        \textbf{Name}      & \textbf{Definition}                & \textbf{Typical usecase}                      \\ \midrule
        Identity           & $x_i$                              & Regression output                             \\ \cmidrule{1-3}
        Sigmoid            & ${1}/{1 + e^{-x_i}}$               & {Hidden layers, binary classification output} \\ \cmidrule{1-3}
        Hyperbolic tangent & $\tanh(x_i)$                       & {Hidden layers, binary classification output} \\ \cmidrule{1-3}
        ReLU               & $\max(0, x_i)$                     & Hidden layers                                 \\ \cmidrule{1-3}
        Leaky ReLU         & $\max(0, x_i) - 0.01\max(0, -x_i)$ & Hidden layers                                 \\ \cmidrule{1-3}
        Softmax            & ${e^x_i}/{\sum_{j=1}^k e^{x_j}}$   & {Multi-class classification output}           \\ \bottomrule
    \end{tblr}
\end{table}

Dense models mean that each node in one layer depends on all nodes in the previous layer.
...
With an unlabalbed dataset, the goal is to generate data that is indistinguishable from the real data.
The first model, the generator, attempts to generate samples from the underlying distribution.
On the other hand, the discriminator is tasked with disguingishing between data produced by the generator and the real samples from the data set.
